export const quizData = [
  {
    id: '1',
    question: 'Wanne daga cikin waɗannan shine tasirin gama-gari na amfani da cocaine?',
    options: [
      'Rage bugun zuciya',
      'Ƙaruwar kuzari da fahimi',
      'Ingantaccen ƙwaƙwalwar ajiya',
      'Rage tashin hankali'
    ],
    correctAnswer: 1,
    explanation: 'Cocaine maganin ƙarfafawa ne wanda ke ƙara kuzari, fahimi, da bugun zuciya.'
  },
  {
    id: '2',
    question: 'Mene ne babban haɗarin da ke tattare da amfani da heroin?',
    options: [
      'Ingantaccen lafiyar jiki',
      'Yawan shan magani na mutuwa',
      'Ingantaccen aikin fahimi',
      'Mafi kyawun tsarin barci'
    ],
    correctAnswer: 1,
    explanation: 'Amfani da heroin yana ɗauke da babban haɗarin yawan shan magani na mutuwa saboda rashin numfashi.'
  },
  {
    id: '3',
    question: 'Wane gabobi ne ya fi shafa ta hanyar cin zarafin barasa na dogon lokaci?',
    options: [
      'Zuciya',
      'Hanta',
      'Huhu',
      'Koda'
    ],
    correctAnswer: 1,
    explanation: 'Hanta ita ce ke da alhakin narkar da barasa kuma ta fi shafa ta hanyar cin zarafi na dogon lokaci.'
  },
  {
    id: '4',
    question: 'Mene ne alamar halayen gama-gari na jarabar magani?',
    options: [
      'Ingantaccen aikin karatu',
      'Ƙarfafa dangantakar iyali',
      'Sakawa alhakin',
      'Mafi kyawun lafiyar jiki'
    ],
    correctAnswer: 2,
    explanation: 'Sakawa alhakin alama ce ta gama-gari ta halayen jaraba.'
  },
  {
    id: '5',
    question: 'Wane magani ne aka sani da haifar da "bakin meth"?',
    options: [
      'Marijuana',
      'Methamphetamine',
      'Cocaine',
      'Heroin'
    ],
    correctAnswer: 1,
    explanation: 'Methamphetamine yana haifar da matsalolin haƙori masu tsanani da aka sani da "bakin meth" saboda bushewar baki, niƙa haƙora, da rashin tsabtar jiki.'
  },
  {
    id: '6',
    question: 'Mene ne babban tasirin magungunan kashe kuzari?',
    options: [
      'Ƙara bugun zuciya da kuzari',
      'Rage aikin kwakwalwa',
      'Haifar da hangen nesa',
      'Haɓaka fahimtar hankali'
    ],
    correctAnswer: 1,
    explanation: 'Masu kashe kuzari suna rage aikin kwakwalwa da aikin tsakiyar tsarin juyayi.'
  },
  {
    id: '7',
    question: 'Wanne daga cikin waɗannan opioid ne na roba mai ƙarfi sau 50-100 fiye da morphine?',
    options: [
      'Heroin',
      'Fentanyl',
      'Codeine',
      'Oxycodone'
    ],
    correctAnswer: 1,
    explanation: 'Fentanyl opioid ne na roba mai matuƙar ƙarfi wanda yake da ƙarfi sau 50-100 fiye da morphine.'
  },
  {
    id: '8',
    question: 'Mene ne haɗarin gama-gari na amfani da MDMA (Ecstasy)?',
    options: [
      'Rage yanayin jiki',
      'Mummunan rashin ruwa da bugun zafi',
      'Ingantaccen aikin ƙwaƙwalwar ajiya',
      'Rage bugun zuciya'
    ],
    correctAnswer: 1,
    explanation: 'MDMA na iya haifar da mummunan rashin ruwa da hauhawar yanayin jiki mai haɗari, wanda ke haifar da bugun zafi.'
  },
  {
    id: '9',
    question: 'Wane magani ne aka samo daga shukar opium poppy?',
    options: [
      'Cocaine',
      'Methamphetamine',
      'Heroin',
      'LSD'
    ],
    correctAnswer: 2,
    explanation: 'An samo heroin daga morphine, wanda ya fito daga shukar opium poppy.'
  },
  {
    id: '10',
    question: 'Mene ne babban amfanin likita na benzodiazepines?',
    options: [
      'Magance ciwo',
      'Magance damuwa da rashin barci',
      'Ƙara kuzari',
      'Rage zazzabi'
    ],
    correctAnswer: 1,
    explanation: 'Ana amfani da benzodiazepines da farko don magance cututtukan tashin hankali da rashin barci.'
  },
  {
    id: '11',
    question: 'Wanne daga cikin waɗannan shine yuwuwar tasiri na dogon lokaci na amfani da marijuana akan samari?',
    options: [
      'Ingantaccen ci gaban fahimi',
      'Rashin ci gaban kwakwalwa',
      'Ingantaccen aikin ƙwaƙwalwar ajiya',
      'Mafi kyawun aikin karatu'
    ],
    correctAnswer: 1,
    explanation: 'Amfani da marijuana a lokacin samartaka na iya lalata ci gaban kwakwalwa kuma ya shafi ƙwaƙwalwar ajiya da koyo.'
  },
  {
    id: '12',
    question: 'Mene ne "ciwon janyewa"?',
    options: [
      'Tasirin farko na shan magani',
      'Alamomin jiki da na hankali lokacin dakatar da amfani da magani',
      'Tsarin zama jaraba',
      'Hanyar ba da magunguna'
    ],
    correctAnswer: 1,
    explanation: 'Ciwon janyewa yana nufin alamomin jiki da na hankali waɗanda ke faruwa lokacin da mutum mai dogaro ya daina amfani da magani.'
  },
  {
    id: '13',
    question: 'Wane magani ne aka fi sani da "acid"?',
    options: [
      'Psilocybin',
      'LSD',
      'PCP',
      'Ketamine'
    ],
    correctAnswer: 1,
    explanation: 'LSD ana kiranta da "acid" kuma maganin ruɗi ne mai ƙarfi.'
  },
  {
    id: '14',
    question: 'Mene ne babban haɗarin haɗa barasa da sauran masu kashe kuzari?',
    options: [
      'Ƙaruwar matakan kuzari',
      'Ingantaccen aikin fahimi',
      'Rashin numfashi da yawan shan magani',
      'Ingantaccen haɗin kai'
    ],
    correctAnswer: 2,
    explanation: 'Haɗa barasa da sauran masu kashe kuzari na iya haifar da mummunan rashin numfashi, wanda ke haifar da koma ko mutuwa.'
  },
  {
    id: '15',
    question: 'Wanne daga cikin waɗannan maganin rarrabuwa ne?',
    options: [
      'Cocaine',
      'Ketamine',
      'Heroin',
      'MDMA'
    ],
    correctAnswer: 1,
    explanation: 'Ketamine maganin sa barci ne mai rarrabuwa wanda zai iya sa masu amfani su ji an raba su da jikinsu da muhallinsu.'
  },
  {
    id: '16',
    question: 'Mene ne "jurewa" a cikin mahallin amfani da magani?',
    options: [
      'Karɓar al\'umma na amfani da magani',
      'Bukatar manyan allurai don cimma tasiri ɗaya',
      'Ƙarfin jurewa illolin gefen magani',
      'Izini na doka don amfani da magunguna'
    ],
    correctAnswer: 1,
    explanation: 'Jurewa yana faruwa lokacin da mutum yana buƙatar ɗaukar manyan allurai na magani don cimma tasirin da ya fara samu.'
  },
  {
    id: '17',
    question: 'Wane magani ne aka haɗa da abubuwan "lalatar kwanan wata"?',
    options: [
      'Cocaine',
      'GHB',
      'Marijuana',
      'LSD'
    ],
    correctAnswer: 1,
    explanation: 'GHB sau da yawa ana haɗa shi da lalatar jima\'i ta hanyar magani saboda tasirinsa na kwantar da hankali da rashin ƙwaƙwalwar ajiya.'
  },
  {
    id: '18',
    question: 'Mene ne babban tasirin magungunan ƙarfafawa?',
    options: [
      'Rage ayyukan jiki',
      'Ƙara aiki a cikin tsakiyar tsarin juyayi',
      'Haifar da hangen nesa',
      'Haifar da barci'
    ],
    correctAnswer: 1,
    explanation: 'Masu ƙarfafawa suna ƙara aiki a cikin tsakiyar tsarin juyayi, wanda ke haifar da ƙaruwar kuzari, fahimi, da bugun zuciya.'
  },
  {
    id: '19',
    question: 'Wanne daga cikin waɗannan ruɗi ne na halitta da ake samu a cikin wasu naman kaza?',
    options: [
      'LSD',
      'Psilocybin',
      'PCP',
      'DMT'
    ],
    correctAnswer: 1,
    explanation: 'Psilocybin wani abu ne na ruɗi na halitta da ake samu a cikin wasu nau\'in naman kaza.'
  },
  {
    id: '20',
    question: 'Mene ne babban haɗarin cin zarafin shan iska?',
    options: [
      'Ingantaccen ƙarfin huhu',
      'Ciwon shan iska na kwatsam',
      'Ingantaccen aikin fahimi',
      'Mafi kyawun haɗin kai'
    ],
    correctAnswer: 1,
    explanation: 'Abubuwan shan iska na iya haifar da ciwon shan iska na kwatsam, inda zuciya ta tsaya ba zato ba tsammani yayin amfani ko bayansa.'
  },
  {
    id: '21',
    question: 'Wane magani ne aka saba bayarwa don ADHD amma sau da yawa ana amfani da shi mara kyau?',
    options: [
      'Xanax',
      'Adderall',
      'Valium',
      'OxyContin'
    ],
    correctAnswer: 1,
    explanation: 'Adderall, maganin ƙarfafawa don ADHD, ana yawan amfani da shi mara kyau don haɓaka fahimi ko dalilai na nishaɗi.'
  },
  {
    id: '22',
    question: 'Menene "naloxone" ake amfani dashi?',
    options: [
      'Magance janyewar barasa',
      'Juyar da yawan shan maganin opioid',
      'Warkar da jarabar magani',
      'Haɓaka tasirin magani'
    ],
    correctAnswer: 1,
    explanation: 'Naloxone magani ne da ake amfani dashi don juyar da yawan shan maganin opioid cikin sauri ta hanyar toshe masu karɓar opioid.'
  },
  {
    id: '23',
    question: 'Wanne daga cikin waɗannan haɗari ne na amfani da steroid na dogon lokaci?',
    options: [
      'Rage ƙwayar tsoka',
      'Harbin zuciya da lalacewar hanta',
      'Ingantaccen haihuwa',
      'Rage tashin hankali'
    ],
    correctAnswer: 1,
    explanation: 'Amfani da steroid na dogon lokaci na iya haifar da matsalolin kiwon lafiya masu mahimmanci ciki har da harbin zuciya, lalacewar hanta, da rashin haihuwa.'
  },
  {
    id: '24',
    question: 'Mene ne babban abin da ke shafar hankali a cikin marijuana?',
    options: [
      'CBD',
      'THC',
      'Opium',
      'Cocaine'
    ],
    correctAnswer: 1,
    explanation: 'THC (tetrahydrocannabinol) shine babban abin da ke shafar hankali a cikin marijuana wanda ke samar da jin daɗin "buguwa".'
  },
  {
    id: '25',
    question: 'Wane magani ne aka sani da haifar da tsoro mai tsanani da halayen tashin hankali?',
    options: [
      'Marijuana',
      'PCP',
      'Barasa',
      'Magungunan opioid na magani'
    ],
    correctAnswer: 1,
    explanation: 'PCP (phencyclidine) sananne ne don haifar da tsoro mai tsanani, hangen nesa, da halayen tashin hankali a cikin masu amfani.'
  },
  {
    id: '26',
    question: 'Mene ne babban haɗarin amfani da cannabinoids na roba?',
    options: [
      'Shakatawa mai laushi kama da marijuana',
      'Matsananin tashin hankali, hauka, da matsalolin zuciya',
      'Ingantaccen aikin fahimi',
      'Ingantaccen aikin jiki'
    ],
    correctAnswer: 1,
    explanation: 'Cannabinoids na roba na iya haifar da matsanancin tashin hankali, hauka, saurin bugun zuciya, da sauran illolin haɗari sabanin marijuana na halitta.'
  },
  {
    id: '27',
    question: 'Wanne daga cikin waɗannan alama ce ta janyewar gama-gari daga opioids?',
    options: [
      'Ƙaruwar kuzari',
      'Ciwon tsoka da tashin zuciya',
      'Ingantaccen ci',
      'Rage tashin hankali'
    ],
    correctAnswer: 1,
    explanation: 'Janyewar opioid yawanci ya haɗa da ciwon tsoka, tashin zuciya, amai, zawo, da ƙwaƙƙwaran maganin ƙwaƙƙwaran.'
  },
  {
    id: '28',
    question: 'Mene ne babban tasirin magungunan ruɗi?',
    options: [
      'Rage ayyukan jiki',
      'Canza fahimta, tunani, da ji',
      'Ƙara kuzari da fahimi',
      'Magance ciwo'
    ],
    correctAnswer: 1,
    explanation: 'Masu ruɗi da farko suna canza fahimta, tunani, da ji, sau da yawa suna sa masu amfani su ga ko ji abubuwan da ba su nan.'
  },
  {
    id: '29',
    question: 'Wane magani ne aka haɗa da "faɗa" a matsayin nau\'in shan taba?',
    options: [
      'Heroin',
      'Cocaine',
      'Methamphetamine',
      'Marijuana'
    ],
    correctAnswer: 1,
    explanation: 'Faɗa wani nau\'in cocaine ne mai shan taba wanda ke haifar da babban bugu mai sauri amma yana da matuƙar saukin kamuwa.'
  },
  {
    id: '30',
    question: 'Mene ne babban haɗarin amfani da magunguna yayin daukar ciki?',
    options: [
      'Ingantaccen ci gaban tayin',
      'Nakasu da matsalolin ci gaba',
      'Ingantaccen lafiyar uwa',
      'Rage ciwon haihuwa'
    ],
    correctAnswer: 1,
    explanation: 'Amfani da magunguna yayin daukar ciki na iya haifar da nakasu, matsalolin ci gaba, da ciwon janyewar jarirai a cikin jariran da aka haifa.'
  },
  {
    id: '31',
    question: 'Wanne daga cikin waɗannan alama ce ta guba na barasa?',
    options: [
      'Magana mai kyau da haɗin kai',
      'Rudani, amai, da suma',
      'Ƙaruwar kuzari da fahimi',
      'Ingantaccen aikin ƙwaƙwalwar ajiya'
    ],
    correctAnswer: 1,
    explanation: 'Alamun guba na barasa sun haɗa da rudani, amai, kamewa, jinkirin numfashi, da suma waɗanda ke buƙatar taimakon likita nan take.'
  },
  {
    id: '32',
    question: 'Mene ne babban haɗarin amfani da "gishirin wanka"?',
    options: [
      'Shakatawa mai laushi',
      'Matsananin tsoro da halayen tashin hankali',
      'Ingantaccen aikin fahimi',
      'Ingantaccen haɗin kai na jiki'
    ],
    correctAnswer: 1,
    explanation: 'Gishirin wanka na iya haifar da matsanancin tsoro, hangen nesa, halayen tashin hankali, da hauhawar yanayin jiki mai haɗari.'
  },
  {
    id: '33',
    question: 'Wane magani ne aka sani da amfani dashi a cikin saitunan likita azaman maganin sa barci?',
    options: [
      'Heroin',
      'Ketamine',
      'Cocaine',
      'LSD'
    ],
    correctAnswer: 1,
    explanation: 'Ketamine ana amfani dashi a likita azaman maganin sa barci amma kuma ana amfani dashi mara kyau don nishaɗi saboda tasirinsa na rarrabuwa.'
  },
  {
    id: '34',
    question: 'Mene ne babban tasirin nicotine akan jiki?',
    options: [
      'Yana aiki azaman mai kashe kuzari',
      'Yana aiki azaman mai ƙarfafawa',
      'Yana haifar da hangen nesa',
      'Yana magance ciwo'
    ],
    correctAnswer: 1,
    explanation: 'Nicotine maganin ƙarfafawa ne wanda ke ƙara bugun zuciya, fahimi, da sakin dopamine a cikin kwakwalwa.'
  },
  {
    id: '35',
    question: 'Wanne daga cikin waɗannan shine yuwuwar sakamako na jarabar magani akan dangantaka?',
    options: [
      'Ƙarfafa dangantakar iyali',
      'Matsalolin dangantaka da warewa',
      'Ingantaccen rayuwar zamantakewa',
      'Mafi kyawun ƙwarewar sadarwa'
    ],
    correctAnswer: 1,
    explanation: 'Jarabar magani sau da yawa tana haifar da matsalolin dangantaka tare da iyali da abokai, warewar zamantakewa, da matsalolin amincewa.'
  },
  {
    id: '36',
    question: 'Menene "rage cutarwa" a cikin manufofin magani?',
    options: [
      'Kawar da duk magunguna gaba ɗaya',
      'Dabarun rage mummunan sakamakon amfani da magani',
      'Ƙarfafa halastar magani',
      'Ƙara samun magunguna'
    ],
    correctAnswer: 1,
    explanation: 'Rage cutarwa yana mai da hankali kan dabarun aiki don rage mummunan sakamakon da ke tattare da amfani da magani, kamar shirin musanya allura da rigakafin yawan shan magani.'
  },
  {
    id: '37',
    question: 'Wane magani ne aka samo daga shukar coca?',
    options: [
      'Heroin',
      'Cocaine',
      'Marijuana',
      'Opium'
    ],
    correctAnswer: 1,
    explanation: 'An samo cocaine daga ganyen shukar coca, \'yan asalin Kudancin Amurka.'
  },
  {
    id: '38',
    question: 'Mene ne babban haɗarin amfani da benzodiazepine na dogon lokaci?',
    options: [
      'Ingantaccen aikin ƙwaƙwalwar ajiya',
      'Dogaro na jiki da janyewa mai haɗari',
      'Ingantattun iyawar fahimi',
      'Rage tashin hankali na dindindin'
    ],
    correctAnswer: 1,
    explanation: 'Amfani da benzodiazepine na dogon lokaci na iya haifar da dogaro na jiki da alamun janyewa masu haɗari ciki har da kamewa.'
  },
  {
    id: '39',
    question: 'Wanne daga cikin waɗannan hanyar gama-gari ce ta gudanar da magani?',
    options: [
      'Watsa ta hanyar telepathic',
      'Shan baki, shan taba, allura',
      'Osmosis ta cikin fata',
      'Sha ta hanyar ji'
    ],
    correctAnswer: 1,
    explanation: 'Hanyoyin gudanar da magunguna na gama-gari sun haɗa da shan baki, shan taba, allura, shan iska, da facin fata.'
  },
  {
    id: '40',
    question: 'Mene ne babban tasirin caffeine akan jiki?',
    options: [
      'Yana aiki azaman mai kashe kuzari',
      'Yana toshe masu karɓar adenosine a cikin kwakwalwa',
      'Yana haifar da hangen nesa',
      'Yana rage bugun zuciya'
    ],
    correctAnswer: 1,
    explanation: 'Caffeine yana aiki ta hanyar toshe masu karɓar adenosine a cikin kwakwalwa, wanda ke haifar da ƙaruwar fahimi da rage gajiya.'
  },
  {
    id: '41',
    question: 'Wane magani ne aka haɗa da "koma baya" bayan amfani na dogon lokaci?',
    options: [
      'Cocaine',
      'LSD',
      'Heroin',
      'Marijuana'
    ],
    correctAnswer: 1,
    explanation: 'LSD na iya haifar da koma baya (Rashin fahimta na dindindin na Mai ruɗi) inda masu amfani suka sake fuskantar tasiri bayan ɗaukar maganin.'
  },
  {
    id: '42',
    question: 'Mene ne babban haɗarin amfani da magunguna tare da tsaftar da ba a sani ba?',
    options: [
      'Tasiri masu daidaito',
      'Ƙarfin da ba a iya faɗi ba da haɗarin yawan shan magani',
      'Tabbacin aminci',
      'Rage farashi'
    ],
    correctAnswer: 1,
    explanation: 'Magunguna tare da tsaftar da ba a sani ba na iya samun ƙarfin da ba a iya faɗi ba, yana ƙara haɗarin yawan shan magani na bazata.'
  },
  {
    id: '43',
    question: 'Wanne daga cikin waɗannan maganin roba ne da aka ƙera don kwaikwayi tasirin marijuana?',
    options: [
      'Cocaine',
      'Cannabinoids na roba',
      'Heroin',
      'LSD'
    ],
    correctAnswer: 1,
    explanation: 'Cannabinoids na roba sinadarai ne da aka yi a dakin gwaje-gwaje da aka ƙera don kwaikwayi tasirin THC amma sau da yawa sun fi haɗari.'
  },
  {
    id: '44',
    question: 'Mene ne babban haɗarin raba allura don allurar magani?',
    options: [
      'Ingantaccen tasirin magani',
      'Yaduwar HIV da hepatitis',
      'Rage ciwon allura',
      'Mafi kyawun sha magani'
    ],
    correctAnswer: 1,
    explanation: 'Raba allura yana ƙara haɗarin yada cututtukan jini kamar HIV da hepatitis B da C.'
  },
  {
    id: '45',
    question: 'Wane magani ne aka sani da sa masu amfani su ji "ƙarfin allahntaka"?',
    options: [
      'Marijuana',
      'PCP',
      'Cocaine',
      'Barasa'
    ],
    correctAnswer: 1,
    explanation: 'PCP na iya sa masu amfani su ji ba za a iya cin nasara ba kuma su nuna ƙarfin allahntaka, sau da yawa yana haifar da halayen haɗari da raunuka.'
  },
  {
    id: '46',
    question: 'Mene ne babban tasirin magungunan opioid akan fahimtar ciwo?',
    options: [
      'Ƙara hankalin ciwo',
      'Tose siginonin ciwo a cikin kwakwalwa',
      'Haifar da ciwo a matsayin illa',
      'Ba su da tasiri akan ciwo'
    ],
    correctAnswer: 1,
    explanation: 'Opioids suna aiki ta hanyar ɗaura da masu karɓa a cikin kwakwalwa da toshe siginonin ciwo, yana ba da maganin ciwo mai ƙarfi.'
  },
  {
    id: '47',
    question: 'Wanne daga cikin waɗannan alama ce ta jiki ta gama-gari na amfani da magani?',
    options: [
      'Idanu masu haske, mai da hankali',
      'Alƙuwa ko ƙunƙuntaccen ɗalibi',
      'Cikakken haɗin kai na jiki',
      'Matsakaicin nauyin jiki'
    ],
    correctAnswer: 1,
    explanation: 'Yawancin magunguna suna haifar da canje-canje a cikin girman ɗalibi - masu ƙarfafawa sau da yawa suna faɗaɗa ɗalibi yayin da opioids ke ƙuntata su.'
  },
  {
    id: '48',
    question: 'Mene ne babban haɗarin amfani da magunguna a haɗe?',
    options: [
      'Rage illolin gefe',
      'Haɗuwa mara kyau da haɗari',
      'Tabbacin aminci',
      'Ingantaccen tasirin warkewa'
    ],
    correctAnswer: 1,
    explanation: 'Haɗa magunguna na iya haifar da haɗuwa mara kyau da haɗari, sau da yawa yana ninka tasirinsu da haɗari.'
  },
  {
    id: '49',
    question: 'Wane magani ne aka haɗa da "bugu" azaman hanyar amfani?',
    options: [
      'Cocaine',
      'Nitrous oxide',
      'Heroin',
      'Marijuana'
    ],
    correctAnswer: 1,
    explanation: 'Nitrous oxide sau da yawa ana amfani dashi don nishaɗi a cikin "bugu" - ƙananan gwangwani da ake amfani da su don ƙara girman feshin whipped cream.'
  },
  {
    id: '50',
    question: 'Mene ne babban manufar shirye-shiryen ilimin magani?',
    options: [
      'Ƙarfafa amfani da magani',
      'Hana cin zarafin magani ta hanyar ilimi',
      'Koyon yadda ake amfani da magunguna lafiya',
      'Ƙara samun magunguna'
    ],
    correctAnswer: 1,
    explanation: 'Shirye-shiryen ilimin magani suna nufin hana cin zarafin magani ta hanyar ba da cikakkun bayanai game da haɗari da sakamako.'
  }
];

// Function to shuffle questions
export const getShuffledQuestions = () => {
  const shuffled = [...quizData].sort(() => Math.random() - 0.5);
  return shuffled.map(question => ({
    ...question,
    options: [...question.options].sort(() => Math.random() - 0.5)
  }));
};